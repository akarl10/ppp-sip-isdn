 ...
 *
 * ptime-only transcoding added by
 * Michael Saxl <mike@mwsys.mine.bz>
 * 
 * mod_clearmode.c -- CLEARMODE Passthru Codec
 *
 */
...
static switch_status_t switch_clearmode_encode(switch_codec_t *codec,
                                                                                  ...
                                                                                  unsigned int *flag)
{
        uint16_t* d = decoded_data;
        uint8_t* e = encoded_data;
        // only from/to CLEARMODE. This might be needed for 20ms to 30ms ptime adaption
        if(codec->implementation->encode != other_codec->implementation->encode)
                return SWITCH_STATUS_FALSE;

        if(*encoded_data_len<decoded_data_len/2)
                return SWITCH_STATUS_FALSE;
        for(int i=0;i<decoded_data_len/2;i++) {
                e[i]=(d[i] & 0xff);
        }
        *encoded_data_len = decoded_data_len / 2;
        *encoded_rate = 8000;
        return SWITCH_STATUS_SUCCESS;
}
                                                                                  ...
                                                                                  unsigned int *flag)
{
        ...
        uint8_t* e = encoded_data;
        // only from/to CLEARMODE. This might be needed for 20ms to 30ms ptime adaption
        if(codec->implementation->decode != other_codec->implementation->decode)
                return SWITCH_STATUS_FALSE;

        if(*decoded_data_len<encoded_data_len*2)
                return SWITCH_STATUS_FALSE;
        for(int i=0;i<encoded_data_len;i++) {
                d[i]=(e[i] & 0xff);
        }
        *decoded_data_len = encoded_data_len * 2;
        *decoded_rate = 8000;
        return SWITCH_STATUS_SUCCESS;
}
